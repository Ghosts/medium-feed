  ...
  getArticles (feedUrl, callback, type) {
    const _parseArticles = function (xml, articles, type) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        const imgTag = article.content.slice(article.content.indexOf("<img"), article.content.indexOf("</figure>"))

        article.image = imgTag.slice(imgTag.indexOf('src="') + 5, imgTag.indexOf('" />'))
